...
func main() {
	...
	if err != nil && err.Error() != "abort" {
		root.PrintUsage()
		core.Println("error while handling:", err)
		os.Exit(1)
	}
}

func offerDefaultConfig() error {
	core.Println("No config file detected. Would you like to create a new one? [Y/n]")
	...
	if strings.ToLower(s.Text()) != "y" && s.Text() != "" {
		core.Println("Understood, have a nice day")
		os.Exit(0)
	}
